...
- 历史指标、信号和人工备注能力保留在后端策略接口，不在主交易台重复展示行情图。
- 行情图由同花顺或 MT5 查看；银禾界面聚焦持仓、平仓、费用、盈亏和审计归档。
...
- 总览同时显示 A股、国内期货的独立本地 Paper 账户及 MT5 Demo；A股和期货尚未接券商实盘账户。
- 策略计划页记录市场、标的、方向、入场/退出条件、止损止盈、风险比例和执行方式；计划默认只观察或由 AI 给建议，不会直接下单。
- MT5 已平交易净盈亏汇总：成交盈亏 + 佣金 + 隔夜利息 + 其他费用；paper 每笔订单保存已实现毛盈亏、模拟费用和扣费后净盈亏。
- 风险档位为本软件提醒（保证金占用率 25/50/80%、净值低于余额 5/10/20%）；后者是当前浮动亏损相对余额，不是历史高水位回撤，也不等同于 Exness 的保证金警告或强平规则。
...
1. “交易总览”集中查看 A股、国内期货和 MT5 Demo 的账户、风险与统一当前持仓；行情图不在银禾内重复展示。
2. “纸面交易”选择市场和标的后进行本地模拟下单、平仓和 Kill Switch 管理。
3. “交割单”按来源筛选 MT5 Demo 成交与本地 Paper 成交，查看数量、费用、毛盈亏和净盈亏。
4. 行情请使用同花顺或 MT5；银禾只保留持仓、平仓和交易归档。历史分析与信号回放接口仍保留在后端，供后续策略任务使用，不作为主界面功能。
5. “策略计划”只保存交易假设和执行闸门。AI 可以依据计划生成入场/平仓建议，但 Paper 或 MT5 Demo 的实际平仓仍需风险检查和人工确认。
6. MT5 仅可连接 Demo；本地纸面交易与 MT5 Demo 分账显示，不会发送到真实券商或期货柜台。

MT5 买卖价和点差由本机已登录的 MT5 Demo 终端读取，页面每 15 秒轮询一次；银禾页面不承担行情图表展示。

如果页面显示“MT5 未连接”，先检查：MT5 终端已经启动并登录 Demo 服务器；`.env` 中 `MT5_DEMO_ENABLED=true`、`MT5_LOGIN`、`MT5_PASSWORD`、`MT5_SERVER` 和 `MT5_PATH` 齐全；若项目运行在单独 worktree，可用环境变量 `MT5_ENV_PATH` 指向保存密钥的本机 `.env`，不要把 `.env` 提交到 Git。

指标、信号、备注、订单和风控的完整技术路线见
[`docs/CN_LIVE_TRADING_TECHNICAL_ROUTE.md`](docs/CN_LIVE_TRADING_TECHNICAL_ROUTE.md)。

XAUUSD 移动止损策略规则与回测计划见
[`docs/XAUUSD_移动止损策略规格_v1.md`](docs/XAUUSD_移动止损策略规格_v1.md)。

独立的 XAUUSD 回测模块、CSV 约定和运行方式见
[`research/xauusd_trailing/README.md`](research/xauusd_trailing/README.md)。
